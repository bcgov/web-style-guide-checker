...
assert.equal(manifest.content_security_policy.extension_pages, "script-src 'self'; object-src 'self';", "Extension pages must declare an explicit CSP without restricting link-check destinations");
...
const compliance = read("COMPLIANCE.yaml");
...
assert.match(background, /chrome\.sidePanel\.onOpened\.addListener[\s\S]*minimizeCmsLiteEditors/, "Opening the checker side panel must restore a maximized CMS Lite editor");
assert.match(background, /chrome\.sidePanel\.onClosed\.addListener[\s\S]*minimizeCmsLiteEditors/, "Closing the checker side panel must restore a maximized CMS Lite editor");
assert.match(background, /hostname\.toLowerCase\(\) !== CMS_LITE_HOSTNAME/, "Automatic editor minimization must be limited to the CMS Lite editor host");
assert.match(background, /world:\s*"MAIN"[\s\S]*\.cke_button__bcgovmaximize/, "CMS Lite editor minimization must run with access to CKEditor's page-world click handler");
...
assert.match(script, /bcgov\.sharepoint\.com[\s\S]*segments\.includes\("_layouts"\)/, "Authenticated SharePoint system pages must be sent to manual review");
...
assert.match(readme, /Apache License 2\.0/, "The README must identify the source-code licence");
...
assert.match(script, /function isExtensionWorkspaceTab\(/, "The side panel must recognize its own full-tab workspace");
assert.match(script, /workspaceSurface \|\| isExtensionWorkspaceTab\(active\)/, "Page actions must keep targeting the source page while the full-tab workspace is active");
assert.match(script, /workspaceSurface \|\| workspaceTabInPanel/, "The side panel must preserve its saved review while the full-tab workspace is active");
assert.match(script, /await refreshPreviewLifecycle\(\{ forceRemote: false \}\)/, "Opening the extension must respect the persisted lifecycle cache");
assert.match(contributing, /npm test/, "Contributing guidance must point to the complete test suite");
assert.match(contributing, /npm run test:ci/, "Contributing guidance must document strict browser testing in CI");
assert.match(workflow, /npm run test:ci/, "Pull requests must run the strict complete test suite");
assert.match(workflow, /playwright install --with-deps chromium/, "CI must install the browser required by the largest test suites");
assert.match(read("tests/run-ci-tests.js"), /REQUIRE_BROWSER_TESTS: "1"/, "Strict test mode must require browser suites");
assert.match(packageManifest.scripts.test, /rendering-security\.test\.js/, "Hostile-text rendering coverage must run in the complete suite");
assert.match(securityPolicy, /private vulnerability reporting/i, "The security policy must provide a private reporting path");
assert.match(compliance, /name: PIA[\s\S]*status: in-progress[\s\S]*name: STRA[\s\S]*status: in-progress/, "PIA and STRA must be recorded accurately as in progress");
assert.ok(fs.statSync(path.join(root, "LICENSE")).size > 10000, "The Apache 2.0 licence must be included");
...
assert.doesNotMatch(html, /Reading-level checks are always included/, "Optional-check settings must avoid explaining checks that cannot be changed here");
assert.match(html, /class="check-row optional-check-row"/, "Optional-check labels must use dedicated description layout");
assert.match(script, /optionalReviewChecksV1/, "Optional contextual review choices must be stored explicitly");
assert.match(script, /Excluded from this scan:/, "Page details must disclose optional checks excluded from a scan");
assert.doesNotMatch(script, /Reading-level checks were still included/, "Excluded-check disclosure must stay focused on checks that were excluded");
...
assert.match(core, /const RULE_VERSION = "1\.3\.3"/);
...
assert.match(css, /\.optional-check-row > span, \.optional-check-row strong, \.optional-check-row small \{ display: block; \}/, "Optional-check descriptions must appear beneath their labels");
